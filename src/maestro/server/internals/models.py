...
class DagORM(Base):
    ...
    executions = relationship(
        "ExecutionORM", back_populates="dag", cascade="all, delete-orphan"
    )
...
class ExecutionORM(Base):
    ...
    tasks = relationship(
        "TaskORM", back_populates="execution", cascade="all, delete-orphan"
    )
...
class TaskORM(Base):
    ...
    is_final = Column(String, nullable=False, default=False)
...
class LogORM(Base):
    ...
    message = Column(Text)
    timestamp = Column(
        DateTime, default=datetime.datetime.utcnow, nullable=False  # ✔ corretto
    )
    thread_id = Column(String)
